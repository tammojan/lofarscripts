...
if len(sys.argv) != 5:
  print 'Usage:',sys.argv[0],' frequency(Hz) MSname minst maxst'
  exit()

minst = int(sys.argv[3])
maxst = int(sys.argv[4])
...
t = pt.table(sys.argv[2]+'/SPECTRAL_WINDOW',readonly=False,ack=True)
t.putcol('REF_FREQUENCY',numpy.array([frequency]))
t.putcol('CHAN_FREQ',numpy.array([[frequency]]))
t.close()

t = pt.table(sys.argv[2],ack=False)
...
t = pt.table(sys.argv[2]+'/ANTENNA',ack=False)
stcol = t.getcol('NAME')
...
sr = lsr.stationresponse(sys.argv[2], useElementResponse=True)
